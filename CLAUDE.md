...
15. **Reuse utility classes for typography — never redeclare them.** When an element is a heading or body text, apply the existing utility class in the markup (`.text-h1`–`.text-h4`, `.text-p1`–`.text-p3`, `.text-ui`, `.text-muted`, `.eyebrow`) instead of re-writing `font-size`/`font-weight`/`line-height`/`letter-spacing` in a scoped `<style>` block. The scoped `<style>` should only hold **layout** (grids, flex, margins, max-width, positioning) and genuinely bespoke component bits. Hybrid model: utility classes carry the type system, scoped CSS carries the layout. Combine both on one element when needed: `<h2 class="text-h2 tb-h2">` where `.tb-h2 { margin: 0 0 var(--space-md); }`. This keeps the type scale in one place and avoids the drift that comes from copy-pasting the same five typography lines into every component. (Note: `.text-*` utilities set type only — color comes from `.text-muted`/`.text-p3` or the body default; margins are never baked in, so add them in scoped CSS.)
16. **Reuse-first: never copy structure into a third place.** Before duplicating markup or a scoped-CSS block that already exists elsewhere, **stop and extract** — a shared component for *identical structure* (same elements in the same order; only data/text differs), a DS class for a shared *look* on varied content (the `.card` / `.media` model). This rule exists because token-compliant duplication is invisible: copies that all use `var(--space-xl)` and `var(--radius-md)` pass Rule #1 yet still drift (e.g. one card keeps an old corner radius after the others change). Tokens prevent *value* drift; this rule prevents *structure* drift. Triggers: (a) the same section/card/list appears in **2+ files** → extract now, don't wait for a third; (b) building several similar pages in one session → build the first, extract the shared skeleton, then compose the rest from it — do **not** generate page 2 by copy-pasting page 1; (c) the only differences between two copies look incidental → assume they're accidental and verify with numbers before preserving any "variant" (most collapse into one rule). When extracting, pass per-instance values as props and keep genuinely one-off bands bespoke (use a `slot` for the exception rather than a pile of boolean flags). If unsure whether something is "identical enough," propose the extraction and its prop shape before writing it.
